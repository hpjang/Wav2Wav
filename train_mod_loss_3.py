...
def train(rank, a, h):
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for (i, batchM), (j, batchF) in zip(enumerate(train_loaderM), enumerate(train_loaderF)):
            ...
            yFyF_p = generatorF.pre_net.magnitude(yFyF)
            yF_p = generatorF.pre_net.magnitude(yF).clone().detach()
            ...
            yMyM_p = generatorM.pre_net.magnitude(yMyM)
            yM_p = generatorM.pre_net.magnitude(yM)
            ...
            yF_p = generatorF.pre_net.magnitude(yF).clone().detach()
            yFyMyF_p = generatorF.pre_net.magnitude(yFyMyF)
            ...
            # L1 Mel-Spectrogram Loss
            yM_p = generatorM.pre_net.magnitude(yM).clone().detach()
            yMyFyM_p = generatorM.pre_net.magnitude(yMyFyM)
