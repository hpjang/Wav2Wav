...
def train(rank, a, h):
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for (i, batchM), (j, batchF) in zip(enumerate(train_loaderM), enumerate(train_loaderF)):
            ...
            yMxF = generatorF.pre_net(yMyF)
            # yMxF = mel_spectrogram(yMyF.squeeze(1), h.n_fft, h.num_mels, h.sampling_rate, h.hop_size, h.win_size,
            #                               h.fmin, h.fmax_for_loss)

            d_fake_F = pgdF(yMxF)
            yFyF = generatorF(yF)
            xF = generatorF.pre_net(yF).clone().detach()
            yFyF_p = generatorF.pre_net.log_magnitude(yFyF)
            yF_p = generatorF.pre_net.log_magnitude(yF).clone().detach()
            ...
            yFxM = generatorM.pre_net(yFyM)
            # yFxM = mel_spectrogram(yFyM.squeeze(1), h.n_fft, h.num_mels, h.sampling_rate, h.hop_size, h.win_size,
            #                               h.fmin, h.fmax_for_loss)

            d_fake_M = pgdM(yFxM)
            yMyM = generatorM(yM)
            xM = generatorM.pre_net(yM).clone().detach()
            yMyM_p = generatorM.pre_net.log_magnitude(yMyM)
            yM_p = generatorM.pre_net.log_magnitude(yM)
            ...
            yF_p = generatorF.pre_net.log_magnitude(yF).clone().detach()
            yFyMyF_p = generatorF.pre_net.log_magnitude(yFyMyF)
            loss_mel = F.l1_loss(yF_p, yFyMyF_p) * a.cycle_loss_lambda
            ...
            # L1 Mel-Spectrogram Loss
            yM_p = generatorM.pre_net.log_magnitude(yM).clone().detach()
            yMyFyM_p = generatorM.pre_net.log_magnitude(yMyFyM)
            loss_mel = F.l1_loss(yM_p, yMyFyM_p) * a.cycle_loss_lambda
